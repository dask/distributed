...
1. A client holds a future pointing to this task (typically via `~.dask.persist`). The data should stay in RAM
...
-----------------------------
Memory usage can be optimized by tweaking worker-side :ref:`memman`.
