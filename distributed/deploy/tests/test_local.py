...
def test_simple(loop):
    with LocalCluster(4, scheduler_port=0, processes=False, silence_logs=False,
                      diagnostics_address=None, loop=loop) as c:
        with Client(c) as e:
            x = e.submit(inc, 1)
            x.result()
...
def test_procs():
    with LocalCluster(2, scheduler_port=0, processes=False, threads_per_worker=3,
                      diagnostics_address=None, silence_logs=False) as c:
...
class LocalTest(ClusterTest, unittest.TestCase):
    Cluster = partial(LocalCluster, silence_logs=False, diagnostics_address=None)
    kwargs = {'diagnostics_address': None}
...
def test_duplicate_clients():
    ...
    c1 = yield Client(processes=False, silence_logs=False, diagnostics_address=9876)
    with pytest.warns(Exception) as info:
        c2 = yield Client(processes=False, silence_logs=False, diagnostics_address=9876)
...
def test_Client_twice(loop):
    with Client(loop=loop, silence_logs=False, diagnostics_address=None) as c:
        with Client(loop=loop, silence_logs=False, diagnostics_address=None) as f:
...
def test_defaults():
    ...
    with LocalCluster(threads_per_worker=_ncores * 2, scheduler_port=0,
                      silence_logs=False, diagnostics_address=None) as c:
        assert len(c.workers) == 1
...
def test_worker_params():
    with LocalCluster(n_workers=2, scheduler_port=0, silence_logs=False,
                      diagnostics_address=None, memory_limit=500) as c:
        assert [w.memory_limit for w in c.workers] == [500] * 2


def test_memory_limit_none():
    with LocalCluster(n_workers=2, scheduler_port=0, silence_logs=False,
                      processes=False,  diagnostics_address=None, memory_limit=None) as c:
        w = c.workers[0]
...
def test_bokeh(loop, processes):
    ...
    with LocalCluster(scheduler_port=0, silence_logs=False, loop=loop,
                      processes=processes, diagnostics_address=0) as c:
...
def test_scale_up_and_down():
    ...
    cluster = yield LocalCluster(0, scheduler_port=0, processes=False,
                                 silence_logs=False, diagnostics_address=None,
...
def test_silent_startup():
    code = """if 1:
        ...
        with LocalCluster(1, diagnostics_address=None, scheduler_port=0):
            sleep(1.5)
        """
...
def test_remote_access(loop):
    with LocalCluster(scheduler_port=0, silence_logs=False,
                      diagnostics_address=None, ip='', loop=loop) as c:
...
def test_memory(loop, n_workers):
    with LocalCluster(n_workers=n_workers, scheduler_port=0, processes=False,
                      silence_logs=False, diagnostics_address=None, loop=loop) as cluster:
...
def test_death_timeout_raises(loop):
    with pytest.raises(gen.TimeoutError):
        with LocalCluster(scheduler_port=0, silence_logs=False,
                          death_timeout=1e-10, diagnostics_address=None,
...
def test_logging():
    ...
    """
    with LocalCluster(1, processes=False, diagnostics_address=None) as c:
...
def test_ipywidgets(loop):
    ...
    with LocalCluster(scheduler_port=0, silence_logs=False, loop=loop,
                      diagnostics_address=False, processes=False) as cluster:
...
def test_scale(loop):
    ...
    with LocalCluster(scheduler_port=0, silence_logs=False, loop=loop,
                      diagnostics_address=False, processes=False, n_workers=0) as cluster:
...
def test_adapt_then_manual(loop):
    ...
    with LocalCluster(scheduler_port=0, silence_logs=False, loop=loop,
                      diagnostics_address=False, processes=False, n_workers=8) as cluster:
        sleep(0.1)
...
def test_local_tls(loop):
    ...
    with LocalCluster(scheduler_port=8786, silence_logs=False, security=security,
                      diagnostics_address=False, ip='tls://0.0.0.0', loop=loop) as c:
