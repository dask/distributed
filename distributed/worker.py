...
class Worker(ServerNode):
    ...
    * **resource_restrictions**: ``{key: {str: number}}``
    ...
    async def heartbeat(self):
        ...
        msg = {"op": "heartbeat-worker", "now": time(), "metrics": self.get_metrics()}
        self.batched_stream.send(msg)

    async def heartbeat_response(self, status, start, end, heartbeat_interval):
        if status == "missing":
            return

        middle = (start + end) / 2
        self.scheduler_delay = end - middle
