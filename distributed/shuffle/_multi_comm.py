...
from distributed.shuffle._multi_file import ShardBuffer
...
#         try:
#             return MultiComm._queues[IOLoop.current()]
#         except KeyError:
#             queue: asyncio.Queue[None] = asyncio.Queue()
#             for _ in range(MultiComm.max_connections):
#                 queue.put_nowait(None)
#             MultiComm._queues[IOLoop.current()] = queue
#             return queue
    ...
    **State**
    ...
    def __init__(
        ...
        super().__init__(
            ...
            concurrency_limit=MultiComm.max_connections,
        )
        self.send = send
        self.total_size = 0
        self.total_moved = 0

    def sizeof(self, obj: list[list]) -> int:
    ...
    async def _process(self, address: str, shards: list, size: int) -> None:
        ...
        with log_errors():
            ...
            # Consider boosting total_size a bit here to account for duplication
            # FIXME / TODO: This thing does not respect max message size
            with self.time("send"):
                await self.send(address, [b"".join(shards)])
