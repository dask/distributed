...
from distributed import Client, wait, Nanny, Worker
...
def test_retire_state_change(loop):
    from itertools import chain
    @gen_cluster(client=True, timeout=1000)
    async def _iter(c, s, a, b):
        import numpy as np
        ...
        for x in range(2):
            ...
            c.gather(step)
        # print(stuff)
        # stuff_sum = c.map(, list(chain(*stuff))[:2])
        # print(stuff_sum)
        # await c.gather(stuff_sum)
        # print(stuff_sum.gather())
        # #await s.replicate(keys=
...
#     with cluster() as (s, [a, b]):
#         worker = Worker
#         with Client(s["address"], loop=loop, Worker=worker) as c:
            # y = c.compute(lambda x: x + 1, range(10000))
            # for x in range(2000):
            #     v = c.compute(lambda x: x*np.random.randint(1000), y)
#             print(a)
#             retire()
